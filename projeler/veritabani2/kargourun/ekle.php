...
try{
 ...
 if($gad!="" ){
$query=$conn->exec("insert into kargo (kargo_ad,katag_id,mal_id,kargo_sahip) VALUES('$gad','$gad1','$gad2','$gad3')");
            ...
            <i class="fa fa-bars"></i>
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Kargo urun Ekle</h1>
                    </div>
...
?>
  ...
  <input type="hidden" value="" id="garson_id" name="id">
    ...
    <label for="kateg">Kargo Kategorisi</label>
...
while ($row = $query->fetch()) {
  echo "<option value=".$row['kat_id']." selected>".$row['kat_ad']."</option>";
   ...
   ?>
    ...
    <label for="marka">Urun mal </label>
...
while ($row = $query->fetch()) {
  echo "<option value=".$row['mal_id']." selected>".$row['mal_ad']."</option>";
}
   ?>
    </select>
    <label for="fname">Kargo sahibi</label>
    <input type="text" id="fiyat" name="fiyat"  placeholder="Sahip">
